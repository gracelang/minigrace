...
method compile(moduleObject, outfile, rm, bt, buildinfo) {
    ...
    if (!util.extensions.contains("NoMain")) then {
        ...
        out("  done = alloc_done();")
        out("  Object tmp_argv = alloc_Lineup();")
        ...
        out("  \}")
        out("  int partcv[] = \{1\};")
        out("  Object argvSequence = callmethod(prelude, \"sequence\", 1, partcv, &tmp_argv);")
        out("  gc_root(argvSequence);")
        out("  module_sys_init_argv(argvSequence);")
