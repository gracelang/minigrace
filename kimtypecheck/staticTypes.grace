...
def aMethodType = object {
    ...
    method fromGctLine(line : String) -> MethodType {
        ...
        if (par > 0) then {
            ...
            parts.add(aMixPart.withName(partName)parameters([]))
        }
...
def anObjectType = object {
    ...
    method fromDType(dtype) -> ObjectType {
        ...
        match(dtype) case { (false) ->
            ...
            def typeMethods : List⟦MethodType⟧ = typeLiteral.methods
    ...
    method dynamic -> ObjectType {
        object {
            ...
            method isConsistentSubtypeOf(_ : ObjectType) -> dynamic { dynamic }
    ...
    method addTo(oType : ObjectType) name(name' : String)
          ...
          -> Done is confidential {
        def parameters = [aParam.ofType(ptype)]
    ...
    def set : ObjectType is public = fromMethods([]) withName("Set")
    def sequence : ObjectType is public = fromMethods([]) withName("Sequence")
    def dictionary : ObjectType is public = fromMethods([]) withName("Dictionary")
    def point : ObjectType is public = fromMethods([]) withName("Point")
    def binding : ObjectType is public = fromMethods([]) withName("Binding")
    def range : ObjectType is public = fromMethods([]) withName("Range")
    ...
    def ifAbsentBlock = blockTaking([])returning(dynamic)
    def stringDoBlock = blockTaking([aParam.ofType(string)])returning(done)
    def stringKeysValuesDoBlock = blockTaking([aParam.ofType(number), aParam.ofType(string)])returning(done)
    ...
    //    addTo(string) name("lastIndexOf()startingAt()ifAbsent") params(string, ifAbsentBlock) returns(number | dynamic)
          ...
          addTo(string) names(["replace","with"]) parts([ [string], [string] ]) returns(string)
          ...
          addTo(string) names(["substringFrom","size"]) parts([ [number], [number] ]) returns(string)
          addTo(string) names(["substringFrom","to"]) parts([ [number], [number] ]) returns(string)
    ...
    addTo(list) names(["at","put"]) parts([ [number], [dynamic] ]) returns(done)
