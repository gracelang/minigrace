    ...
    method visitTypeDec(o) -> Boolean {
        if (isType == false) then {
            ...
            if (node.kind == "op") then {
                ...
                if (node.typeParams != false) then {
                    t := t ++ "&lt;"
                    for (node.typeParams) do { g->
                        t := t ++ g.value
                        if (g != node.typeParams.last) then { t := t ++ ", " }
                    }
                    t := t ++ "&gt;"
                }
...
for (allmodules) do { filename ->
    if (filename.endsWith(".grace")) then {
        ...
        def moduleObject = allfilesparsed.at(counter)
