...
testSuite {
    test "missing — List and sequence" by {
        assert (tc.methodsIn (List) missingFrom (sequence [1, 2])) shouldBe
            "add(_), addAll(_), addAllFirst(_), addFirst(_), addLast(_), at(_)put(_), clear, copy, pop, remove(_), remove(_)ifAbsent(_), removeAll(_), removeAll(_)ifAbsent(_), removeAt(_), removeFirst, removeLast, reverse, sort, and sortBy(_)"
    }

    test "missing — Sequence and string" by {
        assert (tc.methodsIn (Sequence) missingFrom "x") shouldBe
            "asDictionary, into(_), reversed, sorted, sortedBy(_), and values"
    }

    test "coverage – false and Iterable" by {
        assert(tc.protocolOf (false) notCoveredBy (Object)) shouldBe
            "&&(_), &(_), ::(_), ==(_), hash, isMe(_), matches(_), myIdentityHash, not, prefix!, |(_), ||(_), and ≠(_)"
    }
